...
def update_label_lst(label_lst):
    ...
    for label_item in label_lst:
        if label_item != "O":
            ...
            update_label_lst.append("I-{}".format(label_item))
        ...
        if span_labels[tmp_start][tmp_end] >= threshold:
            if tmp_start != tmp_end:
                for i in range(tmp_start + 1, tmp_end):
                    bmes_labels[i] = "I-{}".format(ner_cate)
            else:
...
def flat_ner_performance(pred_start, pred_end, pred_span, gold_start, gold_end, gold_span, ner_cate, label_lst,
    ...
    if dims == 1:
        ...
        # pred_bmes_label, gold_bmes_label: a list of ["B-PER", "I-PER", ]
        ...
        # map a list of string ["B-PER", "I-PER", ] to a list of label index [1, 2, 3]
        ...
        for pred_start_item, pred_end_item, pred_span_item, gold_start_item, gold_end_item, gold_span_item, ner_cate_item in zip(
            ...
            item_pred_bmes_idx, item_gold_bmes_idx, item_pred_bmes_label, item_gold_bmes_label = flat_ner_performance(
                pred_start_item, pred_end_item, pred_span_item, gold_start_item, \
        ...
        # up_label_lst: ["B-PER", "I-PER", "E-PER"]
...
def nested_ner_performance(pred_start, pred_end, pred_span, gold_start, gold_end, gold_span, ner_cate, label_lst,
    ...
    if dims == 1:
        ...
        for pred_start_item, pred_end_item, pred_span_item, gold_start_item, gold_end_item, gold_span_item, ner_cate_item in zip(
            ...
            pred_span_triple, gold_span_triple = nested_ner_performance(pred_start_item, pred_end_item, pred_span_item, gold_start_item, gold_end_item,
...
def flat_ner_decode(pred_start, pred_end, pred_span, ner_cate, label_lst,
                    threshold=0.5, dims=2):
    # transform label index to label tag: "3" -> "PER"
    cate_idx2label = {idx: value for idx, value in enumerate(label_lst)}
    # label_list: ["PER", "ORG", "O", "LOC"]
    ...
    if dims == 1:
        ...
        return pred_bmes_idx, pred_bmes_label

    elif dims == 2:
        pred_bmes_idx_lst = []
        pred_bmes_label_lst = []

        for pred_start_item, pred_end_item, pred_span_item, ner_cate_item in zip(
                pred_start, pred_end,
                pred_span, ner_cate):
            item_pred_bmes_idx, item_pred_bmes_label = flat_ner_decode(
                pred_start_item, pred_end_item, pred_span_item, ner_cate_item, label_lst, dims=1)

            pred_bmes_idx_lst.append(item_pred_bmes_idx)

            pred_bmes_label_lst.append(item_pred_bmes_label)

        return pred_bmes_label_lst